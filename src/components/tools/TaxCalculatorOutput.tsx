...
function Row({
  ...
  return (
    <div
      className={`flex items-start sm:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800/80 py-2.5 text-xs ${
    ...
    >
      <div className="min-w-0 pr-2">
        ...
        {hint && (
          <p className="text-[11px] text-slate-400 dark:text-slate-500 font-normal mt-0.5 leading-tight">
            {hint}
          </p>
        )}
      </div>
      <span className={`font-mono text-right shrink-0 ${colors[tone]}`}>{value}</span>
    </div>
  );
}
...
function Metric({
  ...
  return (
    <div className={`h-full flex flex-col justify-between rounded-2xl border p-3 sm:p-3.5 transition-all shadow-xs ${tones[tone]}`}>
      <div>
        <div className="flex items-center justify-between gap-1">
          <p className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider opacity-70 leading-tight line-clamp-1">{label}</p>
          {icon && <div className="opacity-75 shrink-0">{icon}</div>}
        </div>
        <p className="mt-1 text-sm sm:text-base md:text-lg font-black font-mono tracking-tight">{value}</p>
      </div>
      {subtext && <p className="mt-1 text-[10px] sm:text-[11px] opacity-75 font-medium leading-tight">{subtext}</p>}
    </div>
  );
}
    ...
    <aside className="space-y-5 lg:sticky lg:top-4 self-start">
      ...
      >
        ...
        <div className="relative p-5 sm:p-7 text-white">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto pt-1 sm:pt-0">
              <button
                type="button"
                onClick={download}
                className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 rounded-xl border border-white/15 bg-white/10 hover:bg-white/20 px-3 py-2 text-xs font-bold text-white transition cursor-pointer"
              ...
              <button
                type="button"
                onClick={copy}
                className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 rounded-xl border border-white/15 bg-white/10 hover:bg-white/20 px-3.5 py-2 text-xs font-bold text-white transition cursor-pointer active:scale-95"
              >
          ...
          <div className="mt-5 sm:mt-6 grid grid-cols-2 gap-2 sm:gap-3 border-t border-white/10 pt-4 sm:pt-5 sm:grid-cols-4">
            <div className="rounded-xl bg-white/[0.06] p-2.5 sm:bg-transparent sm:p-0">
              <p className="text-[10px] uppercase tracking-wider text-slate-400 font-bold truncate">Tổng doanh thu</p>
              <p className="mt-0.5 sm:mt-1 font-black font-mono text-xs sm:text-base text-white truncate">
                {money(result.totalRevenue)}
              </p>
            </div>
            <div className="rounded-xl bg-white/[0.06] p-2.5 sm:bg-transparent sm:p-0">
              <p className="text-[10px] uppercase tracking-wider text-slate-400 font-bold truncate">Thuế GTGT ({result.vatRate}%)</p>
              <p className="mt-0.5 sm:mt-1 font-black font-mono text-xs sm:text-base text-slate-200 truncate">
                {money(result.vat)}
              </p>
            </div>
            <div className="rounded-xl bg-white/[0.06] p-2.5 sm:bg-transparent sm:p-0">
              <p className="text-[10px] uppercase tracking-wider text-slate-400 font-bold truncate">Thuế {incomeName}</p>
              <p className="mt-0.5 sm:mt-1 font-black font-mono text-xs sm:text-base text-slate-200 truncate">
                {money(result.incomeTax)}
              </p>
            </div>
            <div className="rounded-xl bg-white/[0.06] p-2.5 sm:bg-transparent sm:p-0">
              <p className="text-[10px] uppercase tracking-wider text-slate-400 font-bold truncate">Còn phải nộp</p>
              <p
                className={`mt-0.5 sm:mt-1 font-black font-mono text-xs sm:text-base truncate ${
      ...
      <section className="overflow-hidden rounded-3xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xs transition-colors p-4 sm:p-5">
        <div className="flex items-center justify-between rounded-2xl bg-slate-100 dark:bg-slate-800 p-1 border border-slate-200/80 dark:border-slate-700/80 text-[11px] sm:text-xs font-bold mb-4 gap-1">
          <button
            type="button"
            onClick={() => setActiveTab("breakdown")}
            className={`flex-1 min-w-0 flex items-center justify-center gap-1 sm:gap-1.5 py-2 px-1.5 rounded-xl transition-all cursor-pointer text-center ${
              activeTab === "breakdown"
          ...
          >
            ...
            <span className="truncate">Chi tiết thuế</span>
          </button>
          <button
            type="button"
          ...
          >
            ...
            <span className="truncate">Đối chiếu sàn</span>
          </button>
          <button
            type="button"
          ...
          >
            ...
            <span className="truncate">Dòng tiền ròng</span>
          </button>
        </div>
