...
export default function KocPlanner({ feeOverrides, feeLoadWarning = false }: { feeOverrides: FeeOverrideRecord[]; feeLoadWarning?: boolean }) {
  ...
  const feeProfile = resolveFeeProfile("tiktok", shopType, category.id, feeOverrides);
  ...
  const calculationInput = useMemo(
    () => ({
      ...
      transactionFeeRate: feeProfile.transactionRate,
      fixedOrderFee: feeProfile.orderProcessingFee,
    }),
    [category.id, feeProfile.commissionRate, feeProfile.orderProcessingFee, feeProfile.transactionRate, input, shopType]
  );
  ...
  const duplicatePlan = (item: SavedPlan) => {
    ...
    const dupProfile = resolveFeeProfile("tiktok", item.shopType, item.categoryId, feeOverrides);
    setInput(dupInput);
    setShopType(item.shopType);
    setCategoryId(item.categoryId);
    const dupCalcInput = {
      ...
      platformCommissionRate: dupProfile.commissionRate,
      transactionFeeRate: dupProfile.transactionRate,
      fixedOrderFee: dupProfile.orderProcessingFee,
    };
            ...
            >
              <div className="space-y-3.5">
                ...
                <ToggleCard
                  ...
                  note={`Hoa hồng ${feeProfile.commissionRate}%, giao dịch ${feeProfile.transactionRate}%, phí đơn ${feeProfile.orderProcessingFee.toLocaleString("vi-VN")}đ · ${feeProfile.sourceName}`}
                />
