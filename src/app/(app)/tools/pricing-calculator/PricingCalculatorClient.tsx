...
export default function PricingCalculatorClient({ feeOverrides }: { feeOverrides: FeeOverrideRecord[] }) {
  ...
  return <div className="mx-auto max-w-7xl pb-12"><GateModals />
    <div className="mb-6">
      ...
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><div className="flex flex-wrap items-center gap-2"><div className="rounded-xl bg-blue-100 dark:bg-blue-950/50 p-2.5 text-blue-700 dark:text-blue-400"><Calculator size={25} /></div><h1 className="text-2xl font-black text-slate-950 dark:text-white">Tính giá bán & lợi nhuận thực tế</h1><span className="rounded-full bg-amber-100 dark:bg-amber-950/60 px-2.5 py-1 text-[10px] font-black text-amber-800 dark:text-amber-300 border border-amber-200/50 dark:border-amber-800/50">DỮ LIỆU {FEE_DATA_VERSION}</span></div><p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Tính riêng phí sàn, thuế, voucher và tổn thất từ đơn hủy, giao thất bại, trả hàng.</p></div>
        <div className="flex rounded-xl bg-slate-200/70 dark:bg-slate-800 p-1 border border-slate-200/50 dark:border-slate-700/50"><button onClick={() => setMode("target")} className={`flex-1 rounded-lg px-4 py-2 text-xs font-bold transition-all ${mode === "target" ? "bg-white dark:bg-slate-700 text-blue-700 dark:text-blue-300 shadow-sm" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}>Tính giá mục tiêu</button><button onClick={() => setMode("audit")} className={`flex-1 rounded-lg px-4 py-2 text-xs font-bold transition-all ${mode === "audit" ? "bg-white dark:bg-slate-700 text-emerald-700 dark:text-emerald-300 shadow-sm" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}>Thẩm định giá bán</button></div>
      </div>
    </div>

    <div className="mb-6 flex w-fit rounded-xl bg-slate-200/70 dark:bg-slate-800 p-1 border border-slate-200/50 dark:border-slate-700/50"><button onClick={() => setWorkspaceMode("single")} className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-bold transition-all ${workspaceMode === "single" ? "bg-white dark:bg-slate-700 text-blue-700 dark:text-blue-300 shadow-sm" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}><Calculator size={16} /> Định giá đơn lẻ</button><button onClick={() => setWorkspaceMode("bulk")} className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-bold transition-all ${workspaceMode === "bulk" ? "bg-white dark:bg-slate-700 text-emerald-700 dark:text-emerald-300 shadow-sm" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}><Table2 size={16} /> Định giá hàng loạt</button></div>
    ...
    {workspaceMode === "bulk" ? <><BulkPricing baseInput={input} feeOverrides={feeOverrides} onSaveAll={saveMany} />{saveNotice && <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800">{saveNotice}</div>}<SavedCalculations history={savedHistory} onOpen={openSaved} onDelete={deleteSaved} onDeleteAll={deleteAllSaved} /></> : <><div className="grid grid-cols-1 gap-6 lg:grid-cols-12"><div className="space-y-5 lg:col-span-5">
      <section className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm transition-colors"><div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/40 px-5 py-4"><h2 className="flex items-center gap-2 font-bold text-slate-900 dark:text-white"><PackageCheck size={18} className="text-blue-600 dark:text-blue-400" /> Sản phẩm và sàn</h2><button onClick={reset} className="flex items-center gap-1.5 text-xs font-bold text-slate-600 dark:text-slate-200 hover:text-rose-600 dark:hover:text-rose-400 bg-white dark:bg-slate-800 hover:bg-rose-50 dark:hover:bg-rose-950/40 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-700 transition-all cursor-pointer shadow-xs"><RotateCcw size={13} className="text-slate-400 dark:text-slate-300" /> Đặt lại</button></div>
        <div className="space-y-4 p-5"><Field label="Tên sản phẩm" hint="bắt buộc"><input ref={productNameRef} value={productName} onChange={(e) => handleName(e.target.value)} aria-invalid={Boolean(productNameError)} aria-describedby={productNameError ? "product-name-error" : undefined} placeholder="Ví dụ: Áo polo nam" className={`w-full rounded-lg border bg-slate-50 dark:bg-slate-800 dark:text-white px-3 py-2 text-sm outline-none focus:ring-2 ${productNameError ? "border-rose-400 focus:border-rose-500 focus:ring-rose-500/15" : "border-slate-200 dark:border-slate-700 focus:border-blue-500 focus:ring-blue-500/15"}`} />{productNameError && <p id="product-name-error" className="mt-1.5 text-xs font-semibold text-rose-600 dark:text-rose-400">{productNameError}</p>}</Field>
          {suggestion && <div className="flex items-center justify-between gap-3 rounded-lg border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/40 p-3 text-xs text-blue-800 dark:text-blue-300"><span className="flex items-center gap-1.5"><Sparkles size={14} /> Gợi ý: <strong>{getCategoryLabel(suggestion)}</strong></span><button onClick={() => applyCategory(suggestion.id)} className="rounded-md bg-blue-600 px-2.5 py-1.5 font-bold text-white">Xác nhận</button></div>}
          <div className="grid grid-cols-2 gap-3"><Field label="Sàn"><select value={input.platform} onChange={(e) => changePlatform(e.target.value as Platform)} className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 dark:text-white px-3 py-2 text-sm font-bold"><option value="shopee">Shopee</option><option value="tiktok">TikTok Shop</option></select></Field><Field label="Loại shop"><select value={input.shopType} onChange={(e) => changeShopType(e.target.value as ShopType)} className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 dark:text-white px-3 py-2 text-sm font-bold"><option value="marketplace">Shop thường</option><option value="mall">Mall</option></select></Field></div>
          <CategorySelector platform={input.platform} shopType={input.shopType} categoryId={selectedCategory.id} onChange={applyCategory} />
          <div className="grid grid-cols-2 gap-3"><Field label="Giá vốn / sản phẩm"><MoneyInput value={input.costPerUnit} onChange={(v) => update("costPerUnit", v)} /></Field><Field label="Số lượng / đơn"><NumberInput value={input.quantity} onChange={(v) => update("quantity", Math.max(1, Math.floor(v)))} suffix="" min={1} step={1} /></Field><Field label="Đóng gói / đơn"><MoneyInput value={input.packagingCost} onChange={(v) => update("packagingCost", v)} /></Field><Field label="Ads"><div className="flex overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700"><select value={input.marketingMode} onChange={(e) => update("marketingMode", e.target.value as CostMode)} className="w-24 border-r border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 dark:text-white px-2 text-xs font-bold"><option value="percent">% GMV</option><option value="fixed">đ/đơn</option></select>{input.marketingMode === "fixed" ? <MoneyInput value={input.marketingValue} onChange={(v) => update("marketingValue", v)} className="rounded-none border-0" /> : <div className="flex-1"><NumberInput value={input.marketingValue} onChange={(v) => update("marketingValue", v)} max={100} /></div>}</div></Field></div>
          {mode === "target" ? <div className="rounded-xl border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/40 p-4"><Field label="Lợi nhuận kỳ vọng mong muốn"><div className="flex overflow-hidden rounded-lg border border-blue-200 dark:border-blue-800 bg-white dark:bg-slate-800"><select value={targetMode} onChange={(e) => setTargetMode(e.target.value as "margin" | "fixed")} className="w-40 border-r border-blue-200 dark:border-blue-800 bg-blue-100 dark:bg-blue-900/60 px-2 text-xs font-bold text-blue-900 dark:text-blue-200"><option value="margin">% doanh thu thực</option><option value="fixed">Tiền / đơn phát sinh</option></select>{targetMode === "fixed" ? <MoneyInput value={targetValue} onChange={setTargetValue} className="rounded-none border-0 bg-white dark:bg-slate-800" /> : <div className="flex-1"><NumberInput value={targetValue} onChange={setTargetValue} max={95} /></div>}</div></Field></div> : <div className="rounded-xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/40 p-4"><Field label="Giá niêm yết cần thẩm định"><MoneyInput value={auditPrice} onChange={setAuditPrice} className="border-emerald-300 dark:border-emerald-700 bg-white dark:bg-slate-800 text-base" /></Field></div>}
          <div className="grid grid-cols-[1fr_auto] gap-3"><button onClick={calculate} className="flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-emerald-600 px-4 py-3 text-sm font-black text-white shadow-lg shadow-blue-500/15 cursor-pointer active:scale-98"><Calculator size={17} /> Tính toán giá bán</button><button onClick={saveCurrent} className="flex items-center justify-center gap-2 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 px-4 py-3 text-sm font-bold text-slate-700 dark:text-slate-200 transition-colors cursor-pointer shadow-xs"><Save size={17} /> {editingId ? "Cập nhật" : "Lưu"}</button></div>
          {saveNotice && <p className="rounded-lg bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 px-3 py-2 text-xs font-semibold text-emerald-800 dark:text-emerald-300">{saveNotice}</p>}
        </div></section>

      <section className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm transition-colors"><button onClick={() => setAdvanced((v) => !v)} className="flex w-full items-center justify-between px-5 py-4 text-left hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors"><span><strong className="flex items-center gap-2 text-sm text-slate-900 dark:text-white"><ReceiptText size={17} className="text-violet-600 dark:text-violet-400" /> Chi phí và rủi ro chi tiết</strong><span className="mt-0.5 block text-xs text-slate-500 dark:text-slate-400">Voucher, Affiliate, thuế, vận chuyển và hoàn hàng</span></span>{advanced ? <ChevronUp size={18} className="text-slate-400" /> : <ChevronDown size={18} className="text-slate-400" />}</button>{advanced && <AdvancedFields input={input} update={update} feeProfile={feeProfile} roundingStep={roundingStep} setRoundingStep={setRoundingStep} toggleProgram={toggleProgram} />}</section>
    </div>
