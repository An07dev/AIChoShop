...
export default function LearnClient({
  ...
  const [mobileTab, setMobileTab] = useState<"lesson" | "playlist">("lesson");
  ...
  return (
    <div className="w-full flex flex-col lg:flex-row gap-4 sm:gap-6 max-w-[1600px] mx-auto pb-8 sm:pb-12">
      {/* VÙNG BÊN TRÁI: KHUNG PHÁT VIDEO & NỘI DUNG BÀI HỌC */}
      <div className="flex-1 min-w-0 space-y-4 sm:space-y-6">
        {/* Khung Trình Phát Video */}
        <div className="bg-slate-950 rounded-xl sm:rounded-2xl aspect-video overflow-hidden relative shadow-xl border border-slate-800">
          {!canWatch ? (
            /* =================== PAYWALL OVERLAY KHI LÀ BÀI VIP VÀ CHƯA NÂNG CẤP =================== */
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-center p-4 sm:p-8 md:p-10 z-20 overflow-y-auto">
              {/* Vòng hào quang vương miện */}
              <div className="relative mb-3 sm:mb-4">
                <div className="w-14 h-14 sm:w-20 sm:h-20 bg-gradient-to-br from-amber-400 via-yellow-500 to-amber-600 rounded-2xl sm:rounded-3xl flex items-center justify-center shadow-[0_0_40px_rgba(245,158,11,0.5)] animate-pulse">
                  <Crown size={30} className="text-white fill-white sm:w-9 sm:h-9" />
                </div>
                <div className="absolute -inset-2 bg-amber-500/20 rounded-full blur-xl -z-10"></div>
              </div>

              {/* Huy hiệu VIP */}
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] sm:text-xs font-black bg-amber-500/20 text-amber-400 border border-amber-500/30 uppercase tracking-widest mb-2.5 sm:mb-3">
                <Lock size={12} /> Nội Dung Dành Riêng Cho Khách VIP
              </span>

              {/* Tiêu đề & Thông điệp chuyển đổi */}
              <h2 className="text-lg sm:text-2xl lg:text-3xl font-black text-white max-w-xl leading-tight mb-1.5 sm:mb-2">
                Mở Khóa Toàn Bộ Lộ Trình & Video Chuyên Sâu
              </h2>
              <p className="text-slate-300 text-xs sm:text-sm max-w-lg mb-4 sm:mb-6 leading-relaxed">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-md w-full mb-4 sm:mb-6 text-left">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-3 w-full sm:w-auto px-4 sm:px-0">
                <Link
                  href="/profile#pricing-section"
                  className="w-full sm:w-auto bg-gradient-to-r from-amber-500 via-yellow-500 to-amber-600 hover:from-amber-400 hover:to-yellow-400 text-slate-950 font-black px-6 sm:px-8 py-3 sm:py-3.5 rounded-xl text-xs sm:text-base transition-all shadow-xl shadow-amber-500/25 flex items-center justify-center gap-2 cursor-pointer active:scale-95"
                >
                ...
                {!isLogged && (
                  <Link
                    href="/login"
                    className="w-full sm:w-auto px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl text-xs sm:text-sm font-bold text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 transition-colors text-center"
                  >
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 p-4 sm:p-6 text-center">
              <Video size={48} className="mb-2 sm:mb-3 opacity-40 text-slate-500" />
              <h3 className="text-base sm:text-lg font-bold text-white mb-1">Video đang được cập nhật</h3>
        ...
        <div className="flex lg:hidden bg-slate-100 dark:bg-slate-800/80 p-1 rounded-xl border border-slate-200 dark:border-slate-700/80 text-xs font-bold w-full">
          <button
            type="button"
            onClick={() => setMobileTab("lesson")}
            className={`flex-1 py-2 px-3 rounded-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer ${mobileTab === "lesson"
              ? "bg-white dark:bg-slate-700 text-brand shadow-xs font-black"
          ...
          <button
            ...
            className={`flex-1 py-2 px-3 rounded-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer ${mobileTab === "playlist"
              ? "bg-white dark:bg-slate-700 text-brand shadow-xs font-black"
              : "text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white"
              }`}
          >
        ...
        <div className={`bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-xl sm:rounded-2xl shadow-xs border border-slate-200 dark:border-slate-800 space-y-4 sm:space-y-6 ${mobileTab === "lesson" ? "block" : "hidden lg:block"
          }`}>
          {/* Header Bài Học */}
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 sm:gap-4 pb-4 sm:pb-5 border-b border-slate-100 dark:border-slate-800">
            <div>
              <div className="flex items-center gap-1.5 sm:gap-2 mb-2 flex-wrap">
                <span className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-[10px] sm:text-xs font-black px-2.5 py-1 rounded-lg">
                  Bài #{activeLesson.order}
                </span>

                {activeLesson.isVIP ? (
                  <span className="inline-flex items-center gap-1 bg-gradient-to-r from-amber-500 to-yellow-500 text-white text-[10px] sm:text-xs font-black px-2.5 sm:px-3 py-1 rounded-lg shadow-xs shadow-amber-500/20">
                    <Crown size={12} className="fill-white" /> VIP PRO
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1 bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-[10px] sm:text-xs font-bold px-2.5 py-1 rounded-lg border border-emerald-200 dark:border-emerald-500/30">
                    <Sparkles size={11} /> BÀI HỌC FREE
                  </span>
                )}

                {isCurrentCompleted && (
                  <span className="inline-flex items-center gap-1 bg-brand-light text-brand text-[10px] sm:text-xs font-bold px-2.5 py-1 rounded-lg border border-brand/20">
            ...
            <button
              ...
              className={`w-full sm:w-auto shrink-0 px-4 py-2.5 rounded-xl font-bold text-xs sm:text-sm transition-all flex items-center justify-center gap-2 cursor-pointer active:scale-95 ${isCurrentCompleted
          ...
          <div>
            <h3 className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-slate-400 mb-2 flex items-center gap-1.5">
              <FileText size={14} /> Ghi Chú & Tóm Tắt Bài Học
            </h3>
            <div className="text-slate-700 dark:text-slate-200 text-xs sm:text-sm leading-relaxed whitespace-pre-wrap bg-slate-50/80 dark:bg-slate-800/80 p-3.5 sm:p-4 rounded-xl border border-slate-100 dark:border-slate-700">
          ...
          {(prevLesson || nextLesson) && (
            <div className="flex items-center justify-between gap-2 pt-3.5 sm:pt-4 border-t border-slate-100 dark:border-slate-800">
              {prevLesson ? (
                <button
                  onClick={() => handleSelectLesson(prevLesson)}
                  className="flex items-center gap-1.5 sm:gap-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:text-brand px-2.5 sm:px-3 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer"
                >
                  <ChevronLeft size={16} />
              ...
              {nextLesson ? (
                <button
                  ...
                  className="flex items-center gap-1.5 sm:gap-2 text-xs font-bold text-brand hover:text-brand-hover px-2.5 sm:px-3 py-2 rounded-lg hover:bg-brand-light transition-colors cursor-pointer ml-auto"
                >
                  <span className="truncate max-w-[130px] sm:max-w-[200px]">
      ...
      <div className={`lg:w-[380px] xl:w-[430px] shrink-0 ${mobileTab === "playlist" ? "block" : "hidden lg:block"
        }`}>
        <div className="bg-white dark:bg-slate-900 rounded-xl sm:rounded-2xl shadow-xs border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col lg:max-h-[calc(100vh-100px)] h-fit lg:sticky lg:top-6">
                    ...
                    return (
                      <li key={lesson.id}>
                        <button
                          ...
                          className={`w-full text-left flex items-start gap-2.5 p-2.5 rounded-xl transition-all cursor-pointer ${isActive
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p
                              className={`text-xs font-bold leading-snug line-clamp-2 ${isActive ? "text-brand" : "text-slate-800 dark:text-slate-200"
                                }`}
                            >
                              {lesson.title}
                            </p>
