...
const root = path.resolve('.data/backups/recovery-audit-upgrade-20260915');
const archive = path.join(root, 'public-before.dump');
const sql = fs.readFileSync('prisma/manual/create_recovery_audit.sql', 'utf8');
...
async function main() {
  ...
  try {
    ...
    if (process.argv[2] === 'backup') {
      ...
      const restoreDb = 'recovery_restore_20260915_test';
      ...
      try {
        ...
        const verification = [];
        for (const [table, expected] of [["PasswordReset",["id","userId","tokenHash","passwordVersion","expiresAt","consumedAt","issuedBy","deliveryStatus","createdAt"]],["AdminAuditLog",["id","actorId","action","targetId","details","createdAt"]]]) {
          const columns = (await db.query("SELECT column_name,data_type FROM information_schema.columns WHERE table_schema=$1 AND table_name=$2", ["public",table])).rows;
          for(const name of expected) if(!columns.some(c=>c.column_name===name)) throw new Error("Missing column "+table+"."+name);
          const rls=(await db.query("SELECT relrowsecurity FROM pg_class WHERE oid=$1::regclass", [`public."${table}"`])).rows[0].relrowsecurity;
          const grants=(await db.query("SELECT grantee FROM information_schema.role_table_grants WHERE table_schema=$1 AND table_name=$2 AND grantee IN ($3,$4,$5)",["public",table,"anon","authenticated","PUBLIC"])).rows;
          if(!rls||grants.length) throw new Error("Unsafe permissions: "+table);
          const indexes=(await db.query("SELECT indexname FROM pg_indexes WHERE schemaname=$1 AND tablename=$2",["public",table])).rows;
          verification.push({table,columns,rls,indexes,publicGrants:false});
        }
        const fk=(await db.query("SELECT convalidated FROM pg_constraint WHERE conrelid=$1::regclass AND contype=$2",["public.\"PasswordReset\"","f"])).rows;
        if(!fk.length||fk.some(c=>!c.convalidated))throw new Error("Missing validated reset user foreign key");
        await db.query('COMMIT');
        fs.writeFileSync(path.join(root, 'migration-result.json'), JSON.stringify({ appliedAt: new Date().toISOString(), target: target.hostname, verification }, null, 2));
        console.log('RECOVERY_AUDIT_MIGRATION_COMMITTED_AND_VERIFIED');
