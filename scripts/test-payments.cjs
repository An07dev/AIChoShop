...
function fixture() {
  let state={events:[],intents:[{id:'intent',userId:'user',paymentCode:'ACS0123456789ABCDEF',planId:'plan',planName:'Month',amount:299000,durationDays:30,currency:'VND',accountNumber:'123456789',expiresAt:new Date(Date.now()+1800000),status:'PENDING'}],users:[{id:'user',isVIP:false,vipExpiresAt:null,isLocked:false}]};
  ...
  const tx={paymentWebhookEvent:model('events'),transaction:model('intents'),user:model('users'),$queryRaw:async(strings,id)=>strings.join('').includes('FROM "User"')?state.users.filter(u=>u.id===id):[]};
