...
const root = path.resolve('.data/backups/payment-upgrade-20260914');
const archive = path.join(root, 'public-before.dump');
const sql = fs.readFileSync('prisma/manual/create_payment_intents.sql', 'utf8');
...
async function main() {
  ...
  try {
    ...
    if (process.argv[2] === 'backup') {
      ...
      const restoreDb = 'payment_restore_20260914_test';
      ...
      try {
        ...
        await db.query('LOCK TABLE "Transaction" IN SHARE ROW EXCLUSIVE MODE');
        const legacy = 'SELECT id,"userId",amount,status,type,"sepayId","createdAt","updatedAt" FROM "Transaction" ORDER BY id';
        const before = JSON.stringify((await db.query(legacy)).rows);
        await db.query(sql.replace(/^BEGIN;\s*$/m, '').replace(/^COMMIT;\s*$/m, ''));
        const after = JSON.stringify((await db.query(legacy)).rows);
        if (before !== after) throw new Error('Legacy transaction data changed');
        const added = (await db.query("SELECT column_name FROM information_schema.columns WHERE table_schema='public' AND table_name='Transaction'")).rows.map(row => row.column_name);
        for (const name of ['paymentCode','planId','planName','durationDays','currency','bankName','accountNumber','accountHolder','expiresAt','paidAt','approvedBy']) if (!added.includes(name)) throw new Error(`Missing column ${name}`);
        await db.query('SELECT id,"transactionId" FROM "PaymentWebhookEvent" LIMIT 0');
        const indexes = (await db.query("SELECT indexname FROM pg_indexes WHERE schemaname='public' AND tablename IN ('Transaction','PaymentWebhookEvent')")).rows;
        await db.query('COMMIT');
        fs.writeFileSync(path.join(root, 'migration-result.json'), JSON.stringify({ appliedAt: new Date().toISOString(), target: target.hostname, legacyTransactionsUnchanged: true, columns: added, indexes }, null, 2));
        console.log('MIGRATION_COMMITTED_LEGACY_DATA_UNCHANGED');
