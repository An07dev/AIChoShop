...
test('admin PostgreSQL: lists, payment snapshots, atomic VIP events and reporting', {skip:!process.env.TEST_DATABASE_URL},async()=>{
 ...
 try{
  ...
  const usersPage=await load('src/app/admin/users/page.tsx').default({searchParams:Promise.resolve({q:'member',vip:'free',size:'10',page:'2',sort:'email'})});const users=usersPage.props.children[1].props.initialUsers;assert.equal(users.length,10);assert.ok(users.every(row=>!row.isVIP));assert.ok(users.some(row=>row.email.includes('member')));
  const lessonPage=await load('src/app/admin/lessons/page.tsx').default({searchParams:Promise.resolve({courseId:'course',status:'DRAFT',size:'10',page:'999'})});const children=lessonPage.props.children;assert.equal(children[2].props.initialLessons.length,5);assert.ok(children[2].props.initialLessons.every(row=>row.status==='DRAFT'));assert.equal(children[2].props.courses[0].maxOrder,30);
